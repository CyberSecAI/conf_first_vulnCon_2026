...
def rename_files():
    ...
    for d in dirs:
        ...
        for filename in files:
            ...
            clean_fn = re.sub(r'[^a-zA-Z0-9]', '', filename.lower().replace(ext, ''))
            
            matched = False
            for vid_id, (title, speaker) in mapping.items():
                clean_title = re.sub(r'[^a-zA-Z0-9]', '', title.lower())
                if clean_fn == clean_title or clean_title.startswith(clean_fn) or clean_fn.startswith(clean_title):
                    new_title = title.replace(':', '：').replace('?', '').replace('"', '').strip()
