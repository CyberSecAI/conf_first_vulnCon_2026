...
# 1. Rebuild README.md from scratch to ensure table correctness
...
for vid_id, (title, speaker) in sorted_sessions:
    ...
    insights_link = f"[Insights](./insights/{safe_filename}.md)"
    transcript_link = f"[Transcript](./transcripts/{safe_filename}.txt)"
...
# 2. Precision Update of individual insight files
...
for filename in os.listdir(insights_dir):
    if filename.endswith('.md'):
        ...
        if matched_speaker:
            ...
            # 2. Reconstruct Executive Summary
            exec_summary_match = re.search(r'## Executive Summary\n(.*?)\n\n## Key Points', content, re.DOTALL)
            if exec_summary_match:
                old_exec = exec_summary_match.group(1)
                
                # Rule: Find the first major verb and replace everything before it with the clean speaker name
                verbs = ['discusses', 'talks about', 'argues', 'explores', 'introduces', 'presents', 'shares', 'outlines']
                new_exec = old_exec
                for verb in verbs:
                    if verb in old_exec.lower():
                        # Use regex to find the verb and replace preceding text
                        pattern = re.compile(rf'^.*?\b{verb}\b', re.IGNORECASE | re.DOTALL)
                        new_exec = pattern.sub(f"{matched_speaker} {verb}", old_exec)
                        break
                
                # Double check for messy subagent additions like "The speaker, [Name], discusses..."
                new_exec = re.sub(r'^The speaker, .*?, ', f"{matched_speaker} ", new_exec)
                
                # Final fallback if verbs didn't catch
                if new_exec == old_exec:
                    new_exec = f"{matched_speaker} presents a deep dive into {official_title}. " + old_exec
