...
## Executive Summary
This panel, featuring Alec Summers (The MITRE Corporation, US), Yogesh Mittal (Red Hat, IN), Lisa Olson (Microsoft, US), Lindsey Cerkovnik (CISA, US), and Jimmy Calderon (TrendAI), dives into the complexities of the CVE record dispute process. The discussion explores the two primary types of disputes—technical disagreements over vulnerability status and operational rule violations regarding scope or counting. The experts highlight the "catch-22" faced by defenders when a CVE is permanently disputed and the unique challenges in the open-source ecosystem where upstream maintainers and downstream vendors may disagree on risk. The panel concludes with a call for community involvement to evolve CVE policies for the high-volume reporting expected in the AI era.
