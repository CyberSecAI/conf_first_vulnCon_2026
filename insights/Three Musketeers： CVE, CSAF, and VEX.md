...
## Executive Summary
Eoin Wilson-Manion (Tharros Labs), Tyler Zellers (Tharros Labs), and Jonathan Spring (CISA, US) detail how CISA operationalizes vulnerability disclosure at a national level using the "Three Musketeers" of security data: CVE, CSAF, and VEX. The speakers emphasize that while machine-readable formats are essential for scalability, the ultimate goal is providing actionable intelligence that clearly communicates vulnerability status and risk. They explore the technical challenges of mapping overlapping standards and the persistent "naming problem" in software, advocating for high-quality, structured data to drive automated defensive responses.
