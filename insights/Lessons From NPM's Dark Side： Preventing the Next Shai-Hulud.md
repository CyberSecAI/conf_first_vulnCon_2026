...
## Executive Summary
Jenn Gile (OpenSourceMalware, US) explores the rising threat of malicious open-source packages within the npm ecosystem, distinguishing them from accidental vulnerabilities. Analyzing major 2025 malware campaigns like Shai-Hulud and Team PCP, Gile highlights how social engineering and account takeovers (ATOs) have become primary attack vectors. The presentation emphasizes that standard vulnerability management—specifically the reflex to immediately upgrade to the latest version—can inadvertently increase susceptibility to malware if the latest release is compromised. Gile provides actionable strategies for both maintainers and consumers, such as utilizing hardware keys, trusted publishing, lock files, and "cooldown periods" to mitigate supply chain risks.
