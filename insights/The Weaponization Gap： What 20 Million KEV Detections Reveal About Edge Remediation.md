...
## Executive Summary
Saeed Abbasi (Qualys Threat Research Unit (TRU), US) analyzes remediation data from over 20 million vulnerability detections to expose the critical "weaponization gap" in edge device security. His research reveals that while edge devices are highly targeted "front doors" to corporate networks, their remediation is frequently hampered by manual processes and maintenance windows, leaving many vulnerabilities open for over a year. Abbasi demonstrates that vendor engineering choices, such as auto-updates, are far more effective than mandates, and highlights the surprising finding that the CISA KEV list primarily serves as a "discovery tool" rather than a prioritization mechanism for the majority of organizations.
