...
## Executive Summary
Craig Heffner (NetRise, US) and Peter Eacmen (NetRise, US) explore critical gaps in traditional Software Composition Analysis (SCA) and SBOM generation, specifically within compiled binaries. They demonstrate how statically linked libraries and vendored code create "hidden dependencies" that standard package managers and manifest-based scanners often fail to detect. By emphasizing the binary as the "ground truth," Heffner and Eacmen show how deep analysis can uncover significant security risks—such as vulnerable versions of OpenSSL embedded in shared objects—that remain invisible to traditional defensive tools.
